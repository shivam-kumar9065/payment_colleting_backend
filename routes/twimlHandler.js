...
router.post("/call-status", async (req, res) => {
  const { CallStatus, To } = req.body;
  if (!CallStatus || !To) return res.sendStatus(400);
  ...
  else if (["no-answer", "busy", "failed", "canceled"].includes(CallStatus)) status = "not-answered";
  else if (["ringing", "in-progress"].includes(CallStatus)) status = "calling";
  else if (CallStatus === "answered") status = "connected";
